...
func (repo *documentrepo) GetAllDocuments(email dto.Email) ([]*dto.Document, error) {
...
func (repo *documentrepo) SearchDocuments(email string, searchQuery string) ([]*dto.Document, error) {
	filter := bson.M{
		"$or": []bson.M{
			{"author": email},
			{"readAccess": email},
...
func (repo *documentrepo) UpdateDocument(documentID string, incomingData dto.DocumentData) error {
...
func (repo *documentrepo) GetDocumentByID(documentID string) (*dto.Document, error) {
...
func (repo *documentrepo) UpdateCollaborators(documentID string, collaborators dto.Access) (dto.Document, error) {
	objectID, err := primitive.ObjectIDFromHex(documentID)
	if err != nil {
		fmt.Print(err)
